...
# ---------------------------------------------------------------
# 2-1. 무거운 자료 미리 읽기 (첫 답변을 앞당기는 곳)
#
#     시간대 자료(timezonefinder)를 처음 읽는 데 0.7초쯤 걸립니다.
#     그냥 두면 그 0.7초가 '첫 사용자의 첫 계산' 에 그대로 붙습니다.
#     사용자가 생년월일을 입력하는 동안 딴 갈래(thread)에서 미리 읽어둡니다.
#
#     [화면을 늦추지 않습니다]
#         daemon 갈래라 화면 그리기를 기다리게 하지 않고,
#         join() 하지 않으므로 이 줄에서 멈추지 않습니다.
#     [한 번만]
#         "이미 했는지" 표시는 astrology 모듈이 들고 있습니다.
#         app.py 는 화면이 다시 그려질 때마다 처음부터 다시 실행되므로
#         (Streamlit 이 새 모듈로 exec 합니다) 여기에 표시를 두면
#         갈래를 매번 새로 만들게 됩니다. import 된 모듈은 살아남습니다.
#     [실패해도 됩니다]
#         warm_up() 안에서 조용히 넘어갑니다. 실제 계산할 때 다시 시도합니다.
# ---------------------------------------------------------------
astrology.warm_up_async()
...
def render_privacy_notice() -> None:
    ...
    st.markdown(
        ...
        "맞춤 답변을 지어내는 외부 서비스로 전달될 수 있단다.</li>"
