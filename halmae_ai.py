...
# ===============================================================
#  2-3. 일 · 커리어 상황 (이 고민을 고른 사람에게만 묻습니다)
#
#  [왜 만들었나]
#      칸 이름이 "취업/커리어" 하나뿐이라, 할매(Gemini)가 사용자를
#      자동으로 '취준생' 으로 짐작했습니다. 실제 사용자는
#      대학생·무직·재직자·이직 준비자·직무 전환 고민자로 다 다릅니다.
#      재직 중인 사람에게 "이력서를 고쳐 써라" 는 말은 빗나간 조언이고,
#      취업을 준비하는 사람에게 "상사와의 갈등" 은 없는 상황을 지어낸 말입니다.
#
#  [이 서비스의 원칙과 같은 결]
#      관계 상태(2-2)와 똑같이 다룹니다.
#          사람이 고른다  →  파이썬이 그대로 넘긴다  →  Gemini 는 해석만 한다
#      Gemini 가 취업 상태를 스스로 정하는 일은 없습니다.
#
#  [올해의 카드와는 무관합니다]
#      카드는 "같은 사람 + 같은 해 = 한 장" 이라, 커리어 상황을
#      stable_key 에도 카드 프롬프트에도 넣지 않습니다. (정책 변경 없음)
# ===============================================================
# 이 고민을 골랐을 때만 커리어 상황을 묻습니다.
#     이름을 "취업/커리어" → "일·커리어" 로 넓혔습니다.
#     '취업' 이라는 낱말이 앞에 있으면 재직자가 자기 칸이 아니라고 느낍니다.
CAREER_CONCERN = "일·커리어"

# 예전에 쓰던 이름들. 저장된 답변·analytics 가 깨지지 않게 함께 인정합니다.
#     analytics 에는 고른 낱말이 그대로 쌓여 있어서, 예전 줄에는
#     "취업/커리어" 가 남아 있습니다. 그 줄도 같은 칸으로 세야 합니다.
LEGACY_CAREER_CONCERNS = ("취업/커리어", "취업·커리어", "취업")

CAREER_QUESTION = "현재 어떤 상황에 가까운가요?"

# 고르지 않았거나 밝히고 싶지 않은 경우에 쓰는 값.
# (라디오를 비워둔 채 제출해도 이 값으로 떨어져, 아무것도 짐작하지 않습니다)
CAREER_UNKNOWN = "말하고 싶지 않아요"
...
# answers dict 에 담기는 칸 이름. 코드에서 이 이름으로 찾습니다.
CAREER_CONTEXT_KEY = "커리어 상황"

# 상황마다 '무엇을 중심으로 해석할지'와 '무엇을 하지 말지'를 못 박아둡니다.
# 화면·프롬프트·테스트가 모두 이 표 하나를 봅니다.
CAREER_POLICIES = {
    "첫 취업·구직 중": (
        "아직 첫 직장을 찾는 중인 사람으로 해석하라. "
        "취업 준비를 밀고 가는 방식, 어떤 직무를 고를지 판단하는 기준, "
        "기회가 왔을 때 알아채고 잡는 방식, 실제로 움직이는 실행 방식을 "
        "중심으로 본다. "
        "**이미 회사에 다닌다고 전제하지 마라.** "
        "상사·동료와의 갈등, 조직 안의 승진·평가, 사내 정치, "
        "'지금 회사를 그만두면' 같은 재직자 전용 이야기는 쓰지 마라. "
        "경력·연차·직급이 있다고 가정하지 마라."
    ),
    "재직 중·직장생활": (
        "지금 회사에 다니고 있는 사람으로 해석하라. "
        "조직생활을 버텨내는 방식, 맡은 역할과 성과를 만드는 방식, "
        "상사·동료와의 관계, 업무에서 쌓이는 스트레스를 다루는 방식, "
        "지금 자리에서의 성장 방향을 중심으로 본다. "
        "**취업준비생 전용 조언은 절대 하지 마라.** "
        "'이력서를 써라', '자기소개서를 고쳐라', '취업 지원을 해라', "
        "'취준을 시작해라', '면접을 준비해라', '첫 직장을 구해라' 같은 말은 "
        "어떤 형태로도 쓰지 마라. 이 사람은 이미 일하고 있다. "
        "이직을 고민한다고 전제하지도 마라 — 그건 다른 칸이다."
    ),
    "이직·퇴사 고민": (
        "지금 회사에 다니면서 옮길지 말지를 재고 있는 사람으로 해석하라. "
        "지금 환경과 이 사람 성향이 어디서 부딪히는지, "
        "옮길지 말지를 가를 판단 기준은 무엇인지, "
        "옮기거나 그만두기 전에 무엇을 먼저 챙겨야 하는지를 중심으로 본다. "
        "**'당장 그만두라' 고 단정하지 마라.** "
        "감정이 상한 상태에서 즉시 퇴사하라는 말은 어떤 형태로도 쓰지 마라. "
        "반대로 '무조건 버텨라' 라고 단정하지도 마라. "
        "결정은 이 사람이 한다. 판단에 쓸 기준과 챙길 것을 주어라. "
        "아직 첫 직장을 못 구한 사람으로 취급하지 마라 — 지금 다니고 있다."
    ),
    "진로·직무 전환 고민": (
        "일의 방향 자체를 바꿀지 고민하는 사람으로 해석하라. "
        "길게 보았을 때 어디로 가는 것이 이 사람과 맞는지, "
        "지금까지 쌓은 강점이 새로운 분야와 어디서 이어지는지, "
        "무엇을 어떻게 탐색하고 옮겨갈지를 중심으로 본다. "
        "당장의 취업 성공·연봉 인상 같은 단기 결과로 좁히지 마라. "
        "지금 재직 중인지 아닌지는 알 수 없으니 **어느 쪽도 전제하지 마라.** "
        "'회사를 그만두고', '지금 다니는 곳에서' 같은 표현을 쓰지 마라."
    ),
    CAREER_UNKNOWN: (
        "이 사람이 일과 관련해 어떤 상태인지 알 수 없다. "
        "취업준비생인지, 재직자인지, 무직인지, 이직 중인지 "
        "**어느 쪽도 추측하지 마라.** "
        "일과 커리어 전반에서 나타나는 성향과, 선택할 때 되풀이되는 "
        "판단 패턴만 해석하라. "
        "특정 상태를 전제로 한 행동(이력서 작성·면접 준비·사직서·승진 협상 등)은 "
        "지령으로 내지 마라. "
        "'취준생', '직장인', '백수', '신입', '경력자', '상사', '동료' 같은 "
        "낱말로 상태를 지어내지 말고, '지금 하는 일', '네 일', "
        "'함께 일하는 사람' 처럼 상태를 가리지 않는 말을 써라."
    ),
}

# 커리어 상황을 바꿔 말하지 못하게 막는 규칙. 관계 상태의 잠금 규칙과 같은 방식입니다.
CAREER_LOCK_RULES = """[커리어 상황 사용 규칙 — 어기면 답변 실패로 본다]
- 위 커리어 상황은 사용자가 직접 고른 확정값이다. 다른 상태를 가정하지 마라.
- 위에 적힌 상황이 아닌 처지를 전제로 한 조언을 하지 마라.
- 상태를 짐작하게 하는 표현을 쓰지 마라.
  (예: "아직 취업을 못 했으니", "회사를 다니고 있으니", "요즘 취업이 어렵지")
- 재직 중이라고 적혀 있으면 취업준비 조언(이력서·자기소개서·지원·면접)을
  절대 내지 마라. 첫 취업 중이라고 적혀 있으면 조직 내 갈등·승진·사내 정치를
  전제하지 마라. 이 두 가지를 뒤바꾸는 것이 가장 흔한 실수다.
- 추가 질문에 커리어 상황이 분명하게 적혀 있으면 그 내용을 함께 참고하라.
  다만 분명하지 않으면 특정 상태로 단정하지 말고, 위에 적힌 상황만 따르라.
- 사주·점성술 값으로 취업 상태를 역추적하려 하지 마라.
  명식에는 지금 회사에 다니는지 구직 중인지가 적혀 있지 않다.
- 행동 지령(3단계)을 낼 때 특히 조심하라. 지령은 실제로 하라는 말이라,
  상황과 어긋나면 그대로 쓸모없는 말이 된다.
  (예: 재직 중이라고 적혀 있는데 "이력서를 다듬어라")"""


def normalize_concern(value) -> str | None:
    """예전 고민 분야 이름을 지금 쓰는 이름으로 맞춰줍니다.

    "취업/커리어" 처럼 예전에 저장된 값도 "일·커리어" 로 봅니다.
    (세션에 남아 있던 답변 · 예전 analytics 줄이 깨지지 않게)
    """
    if value is None:
        return None
    text = str(value).strip()
    if text in LEGACY_CAREER_CONCERNS:
        return CAREER_CONCERN
    return text


def career_context(answers: dict | None) -> str | None:
    """이 사람의 일·커리어 상황. 물어보지 않은 고민이면 None.

    '일·커리어' 를 고른 사람에게만 값이 있습니다.
    고르지 않은 채 제출했으면 '말하고 싶지 않아요' 로 봅니다 —
    비어 있다고 취준생으로 넘겨짚으면 안 되니까요.
    """
    if not answers:
        return None
    if normalize_concern(answers.get("고민 분야")) != CAREER_CONCERN:
        return None
    value = (answers.get(CAREER_CONTEXT_KEY) or "").strip()
    return value if value in CAREER_POLICIES else CAREER_UNKNOWN


def build_career_block(answers: dict | None) -> str:
    """커리어 상황을 프롬프트에 붙일 수 있는 글자로. 해당 없으면 빈 글자.

    이 함수가 커리어 상황이 프롬프트로 나가는 유일한 통로입니다.
    """
    state = career_context(answers)
    if state is None:
        return ""

    return "\n".join([
        "[일·커리어 상황 — 사용자가 직접 고른 확정 입력값]",
        f"- 현재 커리어 상황: {state}",
        f"- 이 상황을 기준으로만 해석한다: {CAREER_POLICIES[state]}",
        "",
        CAREER_LOCK_RULES,
    ])
...
def build_profile_block(
    ...
    # 커리어 상황은 '일·커리어' 를 고른 사람에게만 있습니다. (같은 방식)
    career = build_career_block(answers)
    if career:
        blocks.append(career)
